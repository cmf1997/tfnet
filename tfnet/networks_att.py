...
class DeepATT(Network):
    def __init__(self, *, emb_size, conv_num, conv_size, conv_off, linear_size, full_size, dropouts, **kwargs):
        ...
        self.conv = nn.ModuleList([nn.Conv2d(in_channel,out_channel,(1,30),(1,1)) for in_channel,out_channel in zip(in_channels[:-1],in_channels[1:])]) #deepATT

        self.conv_bn = nn.ModuleList([nn.BatchNorm2d(out_channel) for out_channel in in_channels[1:]])          

        self.lstm = nn.LSTM(in_channels[-1], 512, bidirectional=True)

        self.multihead_attn = nn.MultiheadAttention(embed_dim=in_channels[-1], num_heads=4, kdim=in_channels[-1], vdim=in_channels[-1], batch_first=True)
    ...
    def forward(self, DNA_x, tf_x, **kwargs):
        DNA_x = super(DeepATT, self).forward(DNA_x, tf_x)
        ...
        for index, (conv, conv_bn) in enumerate(zip(self.conv, self.conv_bn)):
            ...
            conv_out = F.max_pool2d(conv_out,(1,15),(1,15))
            conv_out = self.dropout[0](conv_out)


        # ---------------- attention ----------------#
        conv_out = torch.transpose(conv_out,3,1).squeeze(2)
        temp, (_, _) = self.lstm(conv_out)
        conv_out, _  = self.multihead_attn(temp, temp, temp)

        # ---------------------- Output = [batch_size, *, multihead_attn dim] ---------------------- #
