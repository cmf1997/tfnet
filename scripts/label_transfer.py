...
'''
@File : label_transfer.py
@Time : 2024/04/15 14:03:12
@Author : Cmf
@Version : 1.0
@Desc : None
'''
...
def generate_binary_string(length, position_value_dict):
    # Create a list of zeros of the specified length
    binary_list = [random.choice(['0', '1']) for _ in range(length)]
 
    # Set the value at the specified position in dict
    for position, value in position_value_dict.items():
        if position >= 0 and position < length:
            binary_list[position] = value
        else:
            print("Invalid position provided.")
            exit

    # Join the list elements with commas to form the string
...
def main():
    ...
    for model, tfs_model in celltypes.items():
        tfs_len = len(tfs_model)
        tmp = celltypes.copy()
        del tmp[model]
        for target, tfs_target in tmp.items():
            print(f"Transfer {target} label to {model}")
            common_tfs = list(filter(lambda x: x in tfs_model, tfs_target))
            print(f"common: {common_tfs}")
            model_indexs = [ tfs_model.index(i) for i in common_tfs ]
            taraget_indexs = [ tfs_target.index(i) for i in common_tfs ]
            with gzip.open(f"{target}_data_test.txt.gz", "rt") as inf:
                with open(f"from.{target}.to.{model}.data_test.txt", 'w') as p_outf:
                    with open(f"from.{target}.to.{model}.neg.data_test.txt", 'w') as n_outf:
                        for line in inf:
                            chr, start, stop, bind_list  = line.split('\t')
                            bind_list = bind_list.rstrip("\n")
                            bind_int_list = [ int(i) for i in bind_list.split(',') ]
                            bind_list = [ str(i) for i in bind_list.split(',') ]
                            position_value_dict = dict(zip(model_indexs, [ bind_list[i] for i in taraget_indexs]))
                            if sum([ bind_int_list[i] for i in taraget_indexs]) != 0:
                                p_outf.write(f"{chr}\t{start}\t{stop}\t{generate_binary_string(tfs_len, position_value_dict)}\n")
                            else:
                                n_outf.write(f"{chr}\t{start}\t{stop}\t{generate_binary_string(tfs_len, position_value_dict)}\n")
                    p_lines = subprocess.run(f"wc -l from.{target}.to.{model}.data_test.txt", shell=True, capture_output=True, text=True)
                    n_lines = subprocess.run(f"wc -l from.{target}.to.{model}.neg.data_test.txt", shell=True, capture_output=True, text=True)
                    if int(p_lines.stdout.split()[0]) < int(n_lines.stdout.split()[0]):
                        selected_lines_from_file = random.sample(range(int(n_lines.stdout.split()[0])), int(p_lines.stdout.split()[0]))
                    else:
                        selected_lines_from_file = range(int(n_lines.stdout.split()[0]))
                    with open(f"from.{target}.to.{model}.neg.data_test.txt", 'r') as n_outf:
                        content = n_outf.readlines()
                        for line in selected_lines_from_file:
                            p_outf.write(content[line])
                    os.system(f"rm from.{target}.to.{model}.neg.data_test.txt")

    os.system(f"gzip from*.data_test.txt")   
