...
from tfnet.networks_tfnet2 import TFNet2
...
def get_binding_core(data_list, model_cnf, model_path, start_id, num_models, core_len=9):
    ...
    for model_id in range(start_id, start_id + num_models):
        model = Model(TFNet2, model_path=model_path.with_stem(f'{model_path.stem}-{model_id}'), pooling=False,
                      **model_cnf['model'])
...
def main(data_cnf, model_cnf, mode, continue_train, start_id, num_models, allele):
    ...
    if mode == "train":
        ...
        for model_id in range(start_id, start_id + num_models):
            model = Model(TFNet2, model_path=model_path.with_stem(f'{model_path.stem}-{model_id}'), class_weights_dict = class_weights_dict,
                          **model_cnf['model'])
        ...
        for model_id in range(start_id, start_id + num_models):
            ...
            for cv_ in range(5):
                ...
                model = Model(TFNet2, model_path=model_path.with_stem(f'{model_path.stem}-{model_id}-CV{cv_}'), class_weights_dict = class_weights_dict,
                              **model_cnf['model'])
                if not continue_train or not model.model_path.exists():
